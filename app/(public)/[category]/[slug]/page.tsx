...
export default async function PlaceDetailPage({
  ...
  return (
    <div className="container-page">
      ...
      {/* Two-column layout: info left, gallery right */}
      <div className="grid gap-8 pb-16 pt-4 md:grid-cols-2 md:gap-12 md:pb-20">
        {/* LEFT — info */}
        <div className="order-2 md:order-1">
          <div className="mb-4 flex flex-wrap items-center gap-2.5 text-[13px] font-medium text-ink-muted">
            ...
            {place.district ? <span>{place.district}</span> : null}
          </div>

          <h1 className="mb-4 text-[clamp(28px,4vw,44px)] font-extrabold leading-[1.05] tracking-[-0.03em] text-ink">
            {place.name}
          </h1>

          <p className="mb-6 flex items-start gap-2 text-[16px] text-ink-muted">
          ...
          {place.shortDesc ? (
            <>
              <h2 className="mb-3 text-[12px] font-bold uppercase tracking-wider text-ink-light">
                O místě
              </h2>
              <p className="mb-8 text-[17px] leading-[1.7] text-ink">
                {place.shortDesc}
              </p>
            </>
          ) : null}
            ...
            <>
              ...
              <div className="mb-8 flex flex-wrap gap-2">
                {place.tags.map((tag) => (
                  <span
                    key={tag}
                    className="rounded-full bg-surface px-3 py-1.5 text-[13px] font-medium text-ink-muted"
                  >
                    {tag}
                  </span>
                ))}
          ...
          <a
            ...
            className="mb-8 flex w-full items-center justify-center gap-2.5 rounded-xl bg-ink px-5 py-3.5 text-[15px] font-semibold text-white transition hover:-translate-y-0.5 hover:bg-gradient-to-r hover:from-peach-strong hover:to-rose-strong hover:shadow-soft-md sm:w-auto"
          >
            <svg width="16" height="16" viewBox="0 0 24 24" fill="none" stroke="currentColor" strokeWidth="2" strokeLinecap="round" strokeLinejoin="round"><path d="M20 10c0 6-8 12-8 12s-8-6-8-12a8 8 0 0 1 16 0Z"/><circle cx="12" cy="10" r="3"/></svg>
            Navigovat přes Mapy.com
          </a>

          {/* Contacts */}
          {showContacts && c ? (
            <div className="mb-6 rounded-card-lg border border-line bg-card p-6">
              <h4 className="mb-4 text-[12px] font-bold uppercase tracking-wider text-ink-light">
                Kontakty
              </h4>
          ...
          {/* Video — full width inline below the rest of the info */}
          {ytId ? (
            <>
              <h2 className="mb-3 text-[12px] font-bold uppercase tracking-wider text-ink-light">
                Video
              </h2>
              <div className="aspect-video overflow-hidden rounded-card-lg bg-ink">
                <iframe
