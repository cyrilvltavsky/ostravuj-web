...
export function Header({ categories }: { categories: CategoryMeta[] }) {
  ...
  const navLinkClass = (active: boolean) =>
    active
      ? "rounded-xl px-4 py-2.5 text-sm font-semibold text-gradient"
      : "rounded-xl px-4 py-2.5 text-sm font-medium text-ink-muted transition-colors hover:text-gradient";
    ...
    <header className="sticky top-0 z-50 border-b border-line bg-card/85 backdrop-blur-xl backdrop-saturate-150">
        ...
        <nav className="hidden items-center gap-1 md:flex">
          <Link
            ...
            className={`flex items-center justify-center rounded-xl p-2.5 transition-colors ${
              pathname === "/"
                ? "text-peach-strong"
                : "text-ink-muted hover:text-peach-strong"
            }`}
            aria-label="Domů"
          >
        ...
        <div className="flex items-center gap-2">
          ...
          >
            {open ? (
              ...
              <svg width="18" height="18" viewBox="0 0 24 24" fill="none" stroke="currentColor" strokeWidth="2" strokeLinecap="round" strokeLinejoin="round"><path d="M4 6h16"/><path d="M4 12h16"/><path d="M4 18h16"/></svg>
            )}
          </button>
        </div>
      </div>
      ...
      {open && (
        <nav className="absolute left-0 right-0 top-[72px] z-40 flex flex-col gap-1 border-b border-line bg-card px-6 pb-6 pt-4 shadow-soft-lg md:hidden">
          <Link
            href="/"
            className="rounded-xl px-4 py-3.5 text-base font-medium text-ink-muted hover:text-gradient"
          >
            Domů
          </Link>
          {categories.map((cat) => (
